...
with tf.Session() as sess: 
  with tf.device('/gpu:0'):
    ...
    c = tf.constant(np.ones((8,8,3,4), dtype = np.float32))
    result = deform_conv_op.deform_conv_op(a, b, c, strides=[1, 1, 1, 1], rates=[1,1,1,1], padding="VALID", num_groups=3)
    sm = sess.run(result)
    d = tf.constant(np.ones((8,21,3,4), dtype = np.float32))
